...
// The size of packed B matrix(should be a multiple of 64B) is 32B * (GEMM_UNROLL_N^2) * GEMM_LOOP_TIMES_N * GEMM_LOOP_TIMES_K (ifndef NO_REPEAT_C_BLOCK: * GEMM_UNROLL_M_VEC), which should not exceed the capacity of L3 cache per core.
...
//# define NO_REPEAT_C_BLOCK //if the memory bandwidth is not a problem, calculation on every blocked C matrix is not repeated.
